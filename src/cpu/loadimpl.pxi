...
cdef int PUSH_BC(GBCPU cpu) nogil:
    cpu.SP -= 2
    cpu.mem.write16(cpu.SP, cpu.get_BC())
    return 16

cdef int PUSH_DE(GBCPU cpu) nogil:
    cpu.SP -= 2
    cpu.mem.write16(cpu.SP, cpu.get_DE())
    return 16

cdef int PUSH_HL(GBCPU cpu) nogil:
    cpu.SP -= 2
    cpu.mem.write16(cpu.SP, cpu.get_HL())
    return 16

cdef int PUSH_AF(GBCPU cpu) nogil:
    cpu.SP -= 2
    cpu.mem.write16(cpu.SP, cpu.get_AF())
    return 16

cdef int POP_BC(GBCPU cpu) nogil:
    cpu.set_BC(cpu.mem.read16(cpu.SP))
    cpu.SP += 2
    return 12

cdef int POP_DE(GBCPU cpu) nogil:
    cpu.set_DE(cpu.mem.read16(cpu.SP))
    cpu.SP += 2
    return 12

cdef int POP_HL(GBCPU cpu) nogil:
